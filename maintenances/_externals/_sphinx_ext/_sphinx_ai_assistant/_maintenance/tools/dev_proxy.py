...
def _runtime_root() -> pathlib.Path:
    explicit = os.environ.get("SCIKITPLOT_AI_ASSISTANT_RUNTIME", "").strip()
    if explicit:
        candidate = pathlib.Path(explicit).expanduser().resolve()
        if (candidate / "_hf_spaces_proxy" / "_utils").is_dir():
            return candidate
        raise RuntimeError("SCIKITPLOT_AI_ASSISTANT_RUNTIME does not point to the assistant runtime")
    here = pathlib.Path(__file__).resolve()
    for parent in here.parents:
        candidate = parent / "scikitplot" / "_externals" / "_sphinx_ext" / "_sphinx_ai_assistant"
        if (candidate / "_hf_spaces_proxy" / "_utils").is_dir():
            return candidate
    raise RuntimeError("Unable to locate scikitplot/_externals/_sphinx_ext/_sphinx_ai_assistant")


try:  # pragma: no cover - exercised by presence/absence of the runtime checkout
    sys.path.insert(0, str(_runtime_root() / "_hf_spaces_proxy" / "_utils"))
